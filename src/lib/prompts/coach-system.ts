...
Start by acknowledging where the user is (Level ${ctx.currentLevel}: ${levelLabel}), then begin the Understanding Audit at their current level to verify retention before pushing forward. If the user message is exactly "__START_SESSION__", they have just clicked Reinforce or Go deeper; acknowledge their level and start the session with your first question.`}
